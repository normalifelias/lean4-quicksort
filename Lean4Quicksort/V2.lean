...
-/
def Array.qsortn (xs : Array α) (lt : α → α → Bool := by exact (· < ·)) (sllo := 0) (slhi := xs.size) : Array α :=
...
def Vector.qsortn {size} (xs : Vector α size) (lt : α → α → Bool := by exact (· < ·)) (sllo := 0) (slhi := size) : Vector α size :=
  letI : Ord α := ordOfLt lt
  let slhi := min slhi size
  let sllo := min sllo slhi
  quicksorthelper xs sllo slhi (by omega) (by omega)

def Array.qsortOrdn [Ord α] (xs : Array α) (sllo := 0) (slhi := xs.size) : Array α :=
  let slhi := min slhi xs.size
  let sllo := min sllo slhi
  (quicksorthelper xs.toVector sllo slhi (by omega) (by omega)).toArray

def Vector.qsortOrdn [Ord α] {size} (xs : Vector α size) (sllo := 0) (slhi := size) : Vector α size :=
  let slhi := min slhi size
  let sllo := min sllo slhi
  quicksorthelper xs sllo slhi (by omega) (by omega)
