...
| Sort : Array α -> Todo α
| Push : Nat -> α -> Todo α -- used for pushing numbers equal to pivot

def pivotselect [Ord α] (arr : Array α) : Option (α) := -- picks the median of first, middle (rounded down) and last element in array
  if h : arr.size = 0 then none
  else if h : arr.size < 3 then some arr[0] -- if array is smaller than 3 elements, primitively use first element
  else
    let size := arr.size
    let half := size/2
    ...
    if le p1 p2 then -- find median, see cases
      if le p2 p3 then some p2 -- p1 p2 p3 -> middle
      else if le p1 p3 then some p3 -- p1 p3 p2 -> last
      else some p1 -- p3 p1 p2 -> first
    else
      if le p1 p3 then some p1 -- p2 p1 p3 -> first
      else if le p2 p3 then some p3 -- p2 p3 p1 -> last
      else some p2 -- p3 p2 p1 -> middle
...
def pivotsplit [Ord α] (arr : Array α) (pvt : α) : Array α × Nat × Array α := -- initializes pivotsplitHelper with empty le/gt arrays, precalculates array size
...
partial def quicksortHelper [Ord α] (todos : List (Todo α)) (acc : Array α) : Array α := -- recursively splits up arrays based on a pivot and schedules tasks using a todo list
  ...
  | inst :: rest =>
    ...
    | Todo.Sort arr => -- if the instruction is sort, split the array based on a pivot into two more and leaves the pivot to be pushed
      ...
      | some pvt =>
        let (lt, eq, gt) := pivotsplit arr pvt -- split array into less or equal than and greater than a pivot using pivotsplit
        let new := Todo.Sort lt :: Todo.Push eq pvt :: Todo.Sort gt :: rest -- schedule the tasks in todos
...
def quicksort [Ord α] (arr : Array α) : Array α := -- initializes quicksortHelper with empty accumulator
  let acc : Array α := (Array.mkEmpty arr.size) -- sets accumulator size
